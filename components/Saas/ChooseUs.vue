        ...
        <div class="col-lg-4">
          <div class="info">
            <div class="section-head mb-30 style-5">
              <h2>
                {{ rtl ? "" : "Layanan" }}
                <span>{{ rtl ? "خدماتنا" : "Lainnya" }}</span>
              </h2>
            </div>
            <!-- <div class="text mb-50 fs-12px color-666">
              {{
                rtl
                  ? "مع سوق Iteck ، اختر من بين مئات بوابات الدفع لعملائك."
            ...
            <ul>
              <li
                :class="`d-flex ${
                  index !== ChooseUsData.length - 1 ? 'mb-40' : ''
                }`"
                v-for="(item, index) in ChooseUsData"
                :key="index"
              >
            ...
            <NuxtLink
              :to="rtl ? '/rtl-page-services' : '/home'"
              class="btn rounded-pill blue5-3Dbutn hover-blue2 sm-butn fw-bold mt-60 px-5"
            >
              <span>{{ rtl ? "مشاهدة المزيد" : "Lihat Selanjutnya" }}</span>
            </NuxtLink>
          </div>
        </div>
      </div>

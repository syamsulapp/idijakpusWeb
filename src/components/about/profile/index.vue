...
export default {
  ...
  mounted() {
    this.getProfileIdiJakpus();
  },

  computed: {
    data() {
      return this.profileIdi.data;
    },
  },

  methods: {
    async getProfileIdiJakpus() {
      try {
        this.profileIdi.isLoading = true;
        const { data } = await apis.about.profile();
        this.profileIdi.data = data.data;
      } finally {
        this.profileIdi.isLoading = false;
      }
    },
  },
};

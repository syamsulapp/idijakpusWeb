...
export default {
  ...
  methods: {
    async getSejarahIdi() {
      try {
        this.getSejarah.isLoading = true;
        const { data } = await apis.about.sejarah();
        this.getSejarah.data = data.data;
      } finally {
        this.getSejarah.isLoading = false;
      }
    },
  },
};
